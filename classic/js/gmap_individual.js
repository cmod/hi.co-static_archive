...
// HITOTOKI gmap_worldwide javascript toomfoolery
...
// Unselected hitotoki Icon
	var icon;
	icon = new GIcon();
	icon.image = "http://hitotoki.org/img/marker_off.png";
	icon.shadow = "http://hitotoki.org/img/marker_shadow.png";
	...
	iconActive.image = "http://hitotoki.org/img/marker_on.png";
	iconActive.shadow = "http://hitotoki.org/img/marker_shadow.png";
...
//
function load(latlon, longlon, location, issuenumber) {
	if (GBrowserIsCompatible()) {
        map = new GMap2(document.getElementById("individual_map"));
        map.addControl(new customZoomControl());
		map.setCenter(new GLatLng(latlon, longlon), 14);
					
		point = new GLatLng(latlon, longlon);
		map.addOverlay(createMarker(point, location, icon, issuenumber));		
	}
	isloaded = true;
...
//
// Whazzit? : 	
function createMarker(point, location, icon, issue) {
	//var marker = new GMarker(point, icon);
	var marker = new PdMarker(point, icon);
	
	marker.setId(issue);
	marker.setTooltip(location);
...
customZoomControl.prototype.initialize = function(map) {
	...
	imgelement.setAttribute("src", "http://hitotoki.org/img/zoom_in.png");
	...
	var imgelement = document.createElement("img");
	imgelement.setAttribute("src", "http://hitotoki.org/img/zoom_out.png");
